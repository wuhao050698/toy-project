...
def get_vocab(src_file_name, tgt_file_name):
    char2id = {}
    id2char = {}
    tag2id = {START_TAG: 0, STOP_TAG: 1}
    id2tag = {0: START_TAG, 1: STOP_TAG}
...
class BiLSTM_CRF(nn.Module):
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim):
        ...
        self.embedding_dim = embedding_dim  # 词嵌入
        self.hidden_dim = hidden_dim  # 隐藏层
        self.vocab_size = vocab_size  # 词汇大小
        self.tag_to_ix = tag_to_ix  #
        self.tagset_size = len(tag_to_ix)  # 目标大小
...
char2id, id2char, tag2id, id2tag = get_vocab("trainCET.src.c", "trainCET.tgt.c")
X, Y = get_dataset("trainCET.src.c", "trainCET.tgt.c", char2id, tag2id)
VX, VY = get_dataset("validCET.src.c", "validCET.tgt.c", char2id, tag2id)

model = BiLSTM_CRF(len(char2id), tag2id, EMBEDDING_DIM, HIDDEN_DIM)
# model.cuda()
model.to(device)
optimizer = optim.SGD(model.parameters(), lr=0.01)
scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda epoch: 0.9999 ** epoch)
...
for epoch in range(
    ...
    with torch.no_grad():
        ...
        for x, y in zip(VX, VY):
            ...
            total += 1
            correct += (torch.tensor(predicted)[2] == y[2]).sum().item()
    ...
    if a > max_a:
        ...
        print('saving')
        torch.save(data, 'bestModel.m')
    else:
        print('loading')
        data = torch.load('bestModel.m')
