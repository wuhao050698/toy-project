...
class BiLSTM_CRF(nn.Module):
    ...
    def neg_log_likelihood(self, sentence, tags):
        feats = self._get_lstm_features(sentence.to(device))  # lstm
...
# model.cuda()
model.to(device)
optimizer = optim.SGD(model.parameters(), lr=0.01, weight_decay=1e-4)  # 随机梯度下降

# Check predictions before training
with torch.no_grad():
    precheck_sent = torch.LongTensor(X[0])
    precheck_tags = torch.LongTensor(Y[0])
...
for epoch in range(
    ...
    print('epoch %d' % epoch)
    for sentence, tags in zip(X, Y):  # 将X打包成元组
    ...
    with torch.no_grad():
        ...
        for x, y in zip(VX, VY):
            ...
            total += len(x)
            # todo: check LongTensor
            correct += (torch.LongTensor(predicted) == y).sum().item()
